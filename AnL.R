...
AnL <- function(X = Data_X,
  ...
  if (intercept == T) {
    ...
    beta0 = ifelse(family == "gaussian", mean(Y), log(sum(Y == 1) / sum(Y ==
                                                                          0)))
  }
  n = length(Y)
  p = length(X[1, ])
  if (sorted == T) {
    ...
    if (family == "gaussian") {
      importance <- as.vector(apply(X, 2, function(vec) {
        summary(glm(Y ~ vec, family = family1))$coefficients[2, 4]
      }))
  ...
  for (d in ds) {
    ...
    for (m in 1:n) {
      ...
      for (i in 1:(K + 1)) {
        if (i == (K + 1)) {
          if (sum(importance <= 0.05) / d - K > 0 && family == "gaussian") {
            idx_j <- ordered_index[((i - 1) * d + 1):min(sum(importance <= 0.05), i *
                                                           d)]
          } else{
            break
          }
    ...
    if (K == 0) {
      ...
      if (family == "gaussian") {
        ...
        objective_w_expr <- sum(logistic(pred_matrix %*% w) - Y * (pred_matrix %*%
                                                                     w))
      }
      ...
      if (sum1 == F) {
        ...
        result <- solve(Problem(objective_w, constraints = list(w >= 0, sum(w) ==
                                                                  1)))
      }
