...
PMA <- function(X = Data_X,
  ...
  # ALASSO
  pf <- adaptive.weights(x = X,
                         y = Y,
                         weight.method = "univariate")$weights
  trans_X <- X %*% diag((pf)^-1)
  lars_fit <- lars(
    x = X,
    y = Y,
  ...
  if (intercept == F) {
    ...
    for (i in 1:n) {
      ...
      BIC <- c(BIC, ifelse(sum(as.vector(
        which(solutions[i, ] != 0)
      )) == 0, Inf, BIC(lm_fit)))
    }
  } else{
  ...
  is_in_solutions <- function(vec) {
    for (i in 1:n) {
      if (length(vec) == length(as.vector(which(solutions[i, ] != 0)))) {
        if (sum(sort(vec) != sort(as.vector(which(
          solutions[i, ] != 0
        )))) == 0) {
          return(TRUE)
        }
  ...
  if (intercept == F) {
    ...
    lambda = lambda * sum((Y - X %*% beta_matrix[, which.min(BIC)])^2) /
      (n - (length(BIC_model)))
  } else{
    for (i in 1:(length(ordered_index) - 1)) {
      if (!is_in_solutions(ordered_index[1:i])) {
    ...
    lambda = lambda * sum((Y - cbind(1, X) %*% beta_matrix[, which.min(BIC)])^2) /
      (n - (length(BIC_model)))
  }
  ...
  w <- Variable(K)
  objective_w_expr <- cvxr_norm(pred_matrix %*% w - Y)^2 + lambda * sum(v *
                                                                          w)
