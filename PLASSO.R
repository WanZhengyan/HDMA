pflasso<-function(X=Data_X,Y=Data_Y,intercept=T){
  n=length(X[,1])
  p=length(X[1,])
  lasso_fit<-cv.glmnet(x = X,y = Y,family = "gaussian",alpha = 1,intercept=intercept)
...
plasso<-function(X=Data_X,Y=Data_Y,family="binomial",intercept=T,tau=NULL){
  n=length(X[,1])
  p=length(X[1,])
  if(family=="binomial"){
    lasso_fit<-cv.glmnet(x = X,y = Y,family = "binomial",alpha = 1,intercept=intercept)
    beta_lasso<-as.vector(coef(lasso_fit,s="lambda.min"))
    idx<-which(beta_lasso[-1]!=0)
    if(length(idx)>1){
      beta_fit<-as.vector(coef(cv.glmnet(x = X[,idx],y = Y,family = "binomial",alpha = 1,intercept=intercept)))
      if(intercept==T){
        beta=rep(0,p+1)
    ...
    ordered_index <- order(abs(as.vector(coef(fit, lambda = "min"))[-1]),decreasing=T)
