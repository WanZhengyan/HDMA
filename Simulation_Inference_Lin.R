...
simulation<-foreach(setting=1:length(settings[,1]),.combine="rbind",.packages=packages) %dopar% {
  ...
  for(round in 1:500){
    ...
    Data<-Create_data_lin(n,p,Sigma,beta,sd=sd)
    Data_X<-Data[,1:p]
    Data_Y<-Data[,(p+1)]
    ...
    inference_lasso=Inference(Data_X,Data_Y,family="gaussian",beta_hat=beta_lasso,beta_true=beta,alpha=0.05,B=500,idxlist=idxlist)
    ...
    inference_SCAD=Inference(Data_X,Data_Y,family="gaussian",beta_hat=beta_SCAD,beta_true=beta,alpha=0.05,B=500,idxlist=idxlist)
    ...
    inference_MCP=Inference(Data_X,Data_Y,family="gaussian",beta_hat=beta_MCP,beta_true=beta,alpha=0.05,B=500,idxlist=idxlist)
    ...
    # HDMA-lasso
    HDMA_lasso_fit<-HDMA(Data_X,Data_Y,family="gaussian",nest="mix",intercept=F,penalty="Lasso")
    beta_HDMA_lasso<-HDMA_lasso_fit[[1]]
    inference_HDMA_lasso=Inference(Data_X,Data_Y,family="gaussian",beta_hat=beta_HDMA_lasso,beta_true=beta,alpha=0.05,B=500,idxlist=idxlist)
    ...
    # HDMA-SCAD
    HDMA_SCAD_fit<-HDMA(Data_X,Data_Y,family="gaussian",nest="mix",intercept=F,penalty="SCAD")
    beta_HDMA_SCAD<-HDMA_SCAD_fit[[1]]
    inference_HDMA_SCAD=Inference(Data_X,Data_Y,family="gaussian",beta_hat=beta_HDMA_SCAD,beta_true=beta,alpha=0.05,B=500,idxlist=idxlist)
    ...
    # HDMA-MCP
    HDMA_MCP_fit<-HDMA(Data_X,Data_Y,family="gaussian",nest="mix",intercept=F,penalty="MCP")
    beta_HDMA_MCP<-HDMA_MCP_fit[[1]]
    inference_HDMA_MCP=Inference(Data_X,Data_Y,family="gaussian",beta_hat=beta_HDMA_MCP,beta_true=beta,alpha=0.05,B=500,idxlist=idxlist)
...
write.csv(simulation,file="simulation_Inference_Lin.csv",row.names=F)
